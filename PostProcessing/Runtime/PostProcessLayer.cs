...
namespace UnityEngine.Rendering.PostProcessing
{
	...
	public sealed class PostProcessLayer : MonoBehaviour
	{
		...
		/// <summary>
		/// An optional filter for enabling/disabling specific effects on this layer. Useful if you
		/// want to only render a specific effect (like Ambient Occlusion) or exclude an effect
		/// (such as Motion Blur). This is most useful in camera stacking.
		/// </summary>
		public EffectFiltering filterMode;

		/// <summary>
		/// The actual list of effect to be filtered. Names should match the target PostProcessRenderer class names without the 'Renderer' suffix.
		/// </summary>
		...
		void BuildCommandBuffers()
		{
			...
			bool aoSupported = aoSettings.ShouldRenderEffect(context, this);
			...
			bool isScreenSpaceReflectionsActive = ssrSettings.ShouldRenderEffect(context, this);
		...
		void SetLegacyCameraFlags(PostProcessRenderContext context)
		{
			...
			{
				if (bundle.Value.settings.ShouldRenderEffect(context, this))
		...
		public bool HasActiveEffects(PostProcessEvent evt, PostProcessRenderContext context)
		{
			...
			{
				bool enabledAndSupported = item.bundle.settings.ShouldRenderEffect(context, this);
		...
		public bool IsEffectFilteredOut(PostProcessEffectSettings renderer)
		{
			string name = renderer.GetType().Name;
			if(filterMode == EffectFiltering.Whitelist)
			{
				return !filterList.Contains(name);
			}
			else if(filterMode == EffectFiltering.Blacklist)
			{
				return filterList.Contains(name);
			}
			else
		...
		void RenderList(List<SerializedBundleRef> list, PostProcessRenderContext context, string marker)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				if (effect.settings.ShouldRenderEffect(context, this))
				{
		...
		{
			bool autoExpo = GetBundle<AutoExposure>().settings.ShouldRenderEffect(context, this);
